...
description: Use this agent when you need to perform end-to-end testing, validate application functionality, detect bugs, create test automation scripts, analyze test failures, document defects, or coordinate bug fixes with development teams. Examples: (1) User completes a new feature implementation → Assistant: 'Now let me use the e2e-testing-specialist agent to validate the new feature with comprehensive end-to-end tests'; (2) User reports unexpected behavior → Assistant: 'I'll launch the e2e-testing-specialist agent to investigate and document this issue'; (3) User requests quality assurance → Assistant: 'Let me use the e2e-testing-specialist agent to run a full test suite and identify any regressions'; (4) After code review completion → Assistant: 'I'm going to use the e2e-testing-specialist agent to verify the changes work correctly in real-world scenarios
model: sonnet
color: orange
---
